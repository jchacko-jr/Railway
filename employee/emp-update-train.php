<?php
    ...
    if(isset($_POST['update_train']))
    {
            ...
            $query="update orrs_train set name= ?, route = ?, current = ?, destination = ?, time = ?, number = ?, fare = ?, passengers = ? where id = ? fare_payment_code =? ";
            $stmt = $mysqli->prepare($query);
            //bind this parameters
            $rc=$stmt->bind_param('ssssssssi', $name, $route, $current, $destination, $time, $number, $fare, $passengers, $id,$fare_payment_code);
            $stmt->execute();
                if($stmt)
                {
                    $succ = "Train Updated";
